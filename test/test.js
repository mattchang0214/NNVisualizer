...
function softmax(min, max, step) {
    return d3.range(min, max+step, step).map(d => [x(d), y(1)]);
}
...
selectTransform.transform(select);
...
//             .call(d3.axisLeft(y).tickValues(d3.range(-10, 10, 1)).tickSize(3).tickFormat("").tickSizeOuter(0));

// const line = d3.line().curve(d3.curveMonotoneX)(softmax(-10, 10, 1, x, y));
